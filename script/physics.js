define(['exports', './math'], function (exports, _math) {
	'use strict';
	...
	function wavelengthToRgb(wavelengthNanometers) {
	...
	function wavelengthToUncorrectedRgb(w) {
		const ffr = (min, max) => (0, _math.fractionFromRange)(w, min, max);
  	...
  	645: [ 1, 0, 0 ]
  */
		return w < 380 ? [0, 0, 0] : w < 440 ? [ffrDown(380, 440), 0, 1] : w < 490 ? [0, ffr(440, 490), 1] : w < 510 ? [0, 1, ffr(510, 490)] : w < 580 ? [ffr(510, 580), 1, 0] : w < 645 ? [1, ffr(645, 580), 0] : w <= 780 ? [1, 0, 0] : [0, 0, 0];
	}

	function luminanceCorrection(w) {
		const ffr = (min, max) => (0, _math.fractionFromRange)(w, min, max);
		return w < 420 ? 0.3 + 0.7 * ffr(350, 420) : w <= 700 ? 1 : 0.3 + 0.7 * ffr(780, 700);
	}
});